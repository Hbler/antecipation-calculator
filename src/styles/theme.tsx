...
export default function AppTheme({ children }: Props) {
  ...
  useEffect(() => {
    const savedTheme =
      localStorage.getItem("@antecipation_calc:palleteMode") || "light";
    setCurrentTheme(savedTheme as PalletMode);
  }, []);

  const getOppositTheme = useCallback(() => {
    if (currentTheme === "light") {
      localStorage.setItem("@antecipation_calc:palleteMode", "dark");
      return "dark" as PalletMode;
    } else {
      localStorage.setItem("@antecipation_calc:palleteMode", "light");
